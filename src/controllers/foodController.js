...
const searchFoods = async (request, h) => {
    try {
        const { q, verified, limit = 8 } = request.query;
        
        if (!q || q.trim().length === 0) {
            return h.response({
                status: 'fail',
                message: 'Query parameter q (search term) is required'
            }).code(400);
        }
        ...
        console.log('Search params:', { q: searchTerm, verified, limit: pageLimit });
        ...
        query = query
            .where('food_name', '>=', lowerSearchTerm)
            .where('food_name', '<=', lowerSearchTerm + '\uf8ff')
            .orderBy('food_name')
            .limit(pageLimit * 2);
        ...
        const snapshot = await query.get();
        console.log('Search query result count:', snapshot.size);
        
        let foods = [];
        let processedCount = 0;

        snapshot.forEach((doc) => {
            if (foods.length >= pageLimit) return;
            
            const data = doc.data();
            console.log(`Processing search result:`, { id: data.id, food_name: data.food_name });
